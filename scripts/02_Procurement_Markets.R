...
minLogisticsCostSctgPaths <- function(dfsp,iSCTG,paths, naics, recycle_check_file_path){
  ...
  write(x_write, recycle_check_file_path, ncol=length(x_write), append=TRUE)
  return(dfsp)
}
...
minLogisticsCost <- function(df,runmode, naics, recycle_check_file_path){
  ...
  for (iSCTG in unique(df$Commodity_SCTG)){
	...
	if(runmode==0)	{
		...
		df2 <- minLogisticsCostSctgPaths(df1,iSCTG,c(1:2,4:12,14:30,32:45,47:54),naics, recycle_check_file_path)
		if(nrow(df2)>0) {if(pass==0) {
			df_out <- copy(df2)
			pass <- pass + 1
			} else {
		...
		df2 <- minLogisticsCostSctgPaths(df1,iSCTG,c(1:2,4:12,14:30,32:45), naics, recycle_check_file_path)			## include inland water - 03-06-2017
		if(nrow(df2)>0) {if(pass==0) {
			df_out <- copy(df2)
			pass <- pass + 1
			} else {
