...
distchan_calcats <-
  data.table(CHOICE = c("0", "1", "2+", "2+"), CHID = 1:4) #correspondence between choice model alts and target categories int he distchan model
...
mesozone_gcd[, c("Production_lon",
                 "Production_lat",
                 "Consumption_lon",
                 "Consumption_lat") := NULL]# Drop unneeded fields

#Shipment size model: correspondences, clean up calibration data
ShipSize_calcats <-
  data.table(CHOICE = 1:9, CHID = c(rep(1, 5), 2, rep(3, 3))) #correspondence between choice model alts and target categories in the shipment size model
setkey(ShipSize_cal, Commodity_SCTG, WeightCategory)
ShipSize_cal <-
  ShipSize_cal[!Commodity_SCTG %in% c(0, 99), list(Commodity_SCTG, WeightCategory, TonsPct)]
ShipSize_cal[is.na(TonsPct) | TonsPct < 0.0001, TonsPct := 0.0001]
setnames(
  ShipSize_cal,
  c("Commodity_SCTG", "WeightCategory", "TonsPct"),
  c("CATEGORY", "CHOICE", "TARGET")
)
...
shipSize1 <-
  shipSize1[CHOICE < 5]	## drop last value for distribution calculation
ship1 = list()
...
setnames(skims,
         ...
         c("Production_zone", "Consumption_zone"))
#### -- setkey(skims, Production_zone, Consumption_zone)
cskims <-
  skims[, c("Production_zone",
            "Consumption_zone",
            paste0("time", 1:54),
            paste0("cost", 1:54)), with = FALSE]
...
test <-
  cbind(
    melt(
      cskims[, c("Production_zone",
                 "Consumption_zone",
                 paste0("time", mdpaths)), with = F],
      measure.vars = paste0("time", mdpaths),
      variable.name = "timepath",
      value.name = "time"
    ),
    melt(
      cskims[, paste0("cost", mdpaths), with = F],
      measure.vars = paste0("cost", mdpaths),
...
ineligible <-
  test[is.na(cost), list(Production_zone, Consumption_zone, MinPath)]
ineligible[, InEl := 1]
...
sctg[, c("B2", "B3", "B5") := c(
  model$scenvars$LowDiscRate,
  model$scenvars$MedDiscRate,
  model$scenvars$MedDiscRate,
  model$scenvars$MedDiscRate,
  model$scenvars$HighDiscRate
)
[match(
  Category,
  c(
...
sctg[, a := c(
  model$scenvars$LowMultiplier,
  model$scenvars$MediumMultiplier,
  model$scenvars$HighMultiplier
)
[match(Category2,
       c("Functional", "Functional/Innovative", "Innovative"))]]
sctg[, sdQ := c(
  model$scenvars$LowVariability,
  model$scenvars$MediumVariability,
  model$scenvars$HighVariability
)
[match(Category2,
       c("Functional", "Functional/Innovative", "Innovative"))]]
...
sctg[Category == "Bulk natural resource (BNR)", paste0("B0", 3:12) := sctg[Category ==
                                                                             "Bulk natural resource (BNR)", paste0("B0", 3:12), with = FALSE] * 0.1]
sctg[Category == "Bulk natural resource (BNR)", paste0("B0", 13:45) := sctg[Category ==
                                                                              "Bulk natural resource (BNR)", paste0("B0", 13:45), with = FALSE] * 10.0]
...
sctg[Category == "Animals", paste0("B0", c(1:2, 13:30)) := sctg[Category ==
                                                                  "Animals", paste0("B0", c(1:2, 13:30)), with = FALSE] * 10.0]
sctg[Category == "Animals", B031 := B031 * 0.25]

#Category='Intermediate processed goods (IPG)'
sctg[Category == "Intermediate processed goods (IPG)" |
       Category == "Other", paste0("B0", 1:12) := sctg[Category == "Intermediate processed goods (IPG)" |
                                                         Category == "Other", paste0("B0", 1:12), with = FALSE] * 2.0]
sctg[Category == "Finished goods (FG)", paste0("B0", 1:12) := sctg[Category ==
                                                                     "Finished goods (FG)", paste0("B0", 1:12), with = FALSE] * 10.0]
sctg[Category == "Intermediate processed goods (IPG)" |
       Category == "Other", paste0("B0", 14:30) := sctg[Category == "Intermediate processed goods (IPG)" |
                                                          Category == "Other", paste0("B0", 14:30), with = FALSE] * 0.9]
sctg[Category == "Intermediate processed goods (IPG)" |
       Category == "Other", paste0("B0", 31:46) := sctg[Category == "Intermediate processed goods (IPG)" |
                                                          Category == "Other", paste0("B0", 31:46), with = FALSE] * 0.5]

#Category='Finished goods (FG)'
sctg[Category == "Finished goods (FG)", paste0("B0", 31:46) := sctg[Category ==
                                                                      "Finished goods (FG)", paste0("B0", 31:46), with = FALSE] * 0.9]
sctg[Category == "Finished goods (FG)", paste0("B0", c(32:45, 47:50)) := sctg[Category ==
                                                                                "Finished goods (FG)", paste0("B0", c(32:45, 47:50)), with = FALSE] * 0.25]
...
calcLogisticsCost <- function(dfspi, s, path) {
  ...
  j    <-
    model$scenvars$j		#Fraction of shipment that is lost/damaged
  sdLT <- model$scenvars$sdLT	#Standard deviation in lead time
  LT_OrderTime <-
    model$scenvars$LT_OrderTime	#Expected lead time for order fulfillment (to be added to in-transit time)
  #vars from S
  B2 <- s$B2
  B3 <- s$B3
  B5 <- s$B5
  ...
  if (s$Category == "Bulk natural resource (BNR)" &
      path %in% 5:12)
    B0 <- B0 - 0.25 * B0 * dfspi$lssbd
  if (s$Category == "Finished goods (FG)" &
      path %in% c(14:30, 32:38))
    ls <- ls - 0.25 * ls * dfspi$lssbd
  if (s$Category == "Finished goods (FG)" &
      path %in% 39:45)
    ls <- ls - 0.5 * ls * dfspi$lssbd
  ...
  dfspi[, minc := B0 / 1000 * runif(length(pounds)) + #changed scale of B0 -- too large for small shipment flows
          ...
          a * sqrt((LT_OrderTime + time / 24) * ((sdQ * pounds / 2000) ^
                                                   2) + (pounds / 2000) ^ 2 * (sdLT * LT_OrderTime) ^ 2)]
...
minLogisticsCostSctgPaths <- function(dfsp, iSCTG, paths, naics, group, recycle_check_file_path, log_file_path) {
  ...
  dfsp <-
    cbind(
      melt(
        ...
        measure.vars = paste0("time", paths),
        variable.name = "timepath",
        value.name = "time"
      ),
      melt(
        dfsp[, paste0("cost", paths), with = F],
        measure.vars = paste0("cost", paths),
        variable.name = "costpath",
        value.name = "cost"
      )
    )
  ...
  CAP1FTL      <-
    model$scenvars$CAP1FTL		#Capacity of 1 Full Truckload
  CAP1Airplane <-
  ...
  dfsp[path %in% c(14, 19:26, 31) &
         weight < CAP1FTL, avail := FALSE] #Eliminate FTL and FTL-IMX combinations from choice set if Shipment Size < 1 FTL
  dfsp[path %in% c(32:38) &
         weight < CAP1FTL, avail := FALSE] #Eliminate FTL Linehaul with FTL external drayage choice set if Shipment Size < 1 FTL (Heither, 11-06-2015)
  dfsp[path %in% c(15:18, 27:30, 39:46) &
         weight > CAP1FTL, avail := FALSE] #Eliminate LTL and its permutations from choice set if Shipment Size > FTL (Heither, 11-06-2015)
  dfsp[path %in% 47:50 &
         weight > CAP1Airplane, avail := FALSE] #Eliminate Air from choice set if Shipment Size > Air Cargo Capacity
  dfsp[path %in% 51:52 &
         weight < (0.75 * CAP1FTL), avail := FALSE] #Eliminate Container-Direct from choice set if Shipment Size < 1 40' Container
  dfsp[path %in% 53:54 &
         weight < CAP1FTL, avail := FALSE] #Eliminate International Transload-Direct from choice set if Shipment Size < 1 FTL
  dfsp <-
    dfsp[avail == TRUE &
           !is.na(minc)]		## limit to only viable choices before finding minimum path
  dfsp <-
    dfsp[dfsp[, .I[which.min(minc)], by = list(SellerID, BuyerID)]$V1, ]
  dfsp <-
  ...
  for (iSCTG in unique(df$Commodity_SCTG)) {
    ...
    if (runmode == 0)	{
      ...
      if (nrow(df2) > 0) {
        if (pass == 0) {
          df_out <-
            copy(df2)			### make an actual copy, not just a reference to df2
          pass <- pass + 1
        } else {
...
create_pmg_sample_groups <- function(naics, groups, sprod, log_file_path) {
  ...
  prodconsratio <-
    sum(prodc$OutputCapacityTons) / sum(consc$PurchaseAmountTons)
  if (prodconsratio < 1.1) {
    #TODO need to move this to variables
  ...
  if (sprod == 1) {
    prodconsgroup <-
      merge(prodc[, list(OutputCapacityTons = sum(OutputCapacityTons),
                         Producers = .N), by = group],
            consc[, list(PurchaseAmountTons = sum(PurchaseAmountTons),
                         Consumers = .N), by = group], by = "group")
    ...
    iter <-
      1 #counter to break in case something goes wrong and we get into an endless loop
    ...
    while (nrow(prodconsgroup[prodconsratio < 1]) > 0) {
      ...
      maxgroupprod <-
        prodconsgroup[Producers > 1][which.max(prodconsratio)]$group
      ...
      if (maxsample > 0) {
        ...
        constomove <-
          consc[group == mingroup][sampsellers, list(BuyerID, PurchaseAmountTons)]
        constomove[, PATCum := cumsum(PurchaseAmountTons)]
        threshold <- sum(constomove$PurchaseAmountTons) - reqtomove
        consc[BuyerID %in% constomove[PATCum > threshold]$BuyerID, group :=
                maxgroup]
      } else {
        ...
        prodstomove <-
          prodc[group == maxgroupprod][sampbuyers, list(SellerID, OutputCapacityTons)]
        prodstomove[, OCTCum := cumsum(OutputCapacityTons)]
        threshold <- sum(prodstomove$OutputCapacityTons) - reqtomove
        prodc[SellerID %in% prodstomove[OCTCum > threshold]$SellerID, group :=
                mingroup]
      }
      prodconsgroup <-
        merge(prodc[, list(OutputCapacityTons = sum(OutputCapacityTons),
      ...
      iter <- iter + 1
      if (iter == 100) {
        #break out of the loop. This should never be necessary but here to stop endless loops. Groups with excess consumption requirements will potentially run slowly
        break
      }
    }
  }
  ...
  if (sprod == 0)
    consc[, SingleSourceMaxFraction := 1.0]

}

predict_logit <- function(df,
                          mod,
                          cal = NULL,
                          calcats = NULL,
                          iter = 1) {
  ...
  if (is.numeric(df$CATEGORY))
    df[, CATEGORY := paste0("x", CATEGORY)] #numeric causes problems with column names
  ...
  for (iters in 1:iter) {
    ...
    if (iters > 1 & !is.null(cal) & !is.null(calcats)) {
      ...
      sim <-
        sapply(cats, function(x)
          tabulate(simchoice[min(df$Start[df$CATEGORY == x]):max(df$Fin[df$CATEGORY ==
                                                                          x])], nbins = alts))
      sim <- sim / colSums(sim)

      if (length(unique(calcats$CHOICE)) < length(unique(calcats$CHID))) {
        #the sim choices need to be aggregated to the calibration data
        sim <- cbind(calcats, sim)
        sim <-
          melt(sim,
               id.vars = c("CHOICE", "CHID"),
               variable.name = "CATEGORY")
        sim <- sim[, list(MODEL = sum(value)), by = list(CHOICE, CATEGORY)]
        sim <- merge(sim, cal, c("CATEGORY", "CHOICE"))
        sim[, ascadj := log(TARGET / MODEL)]
        adj <-
          merge(sim, calcats, "CHOICE", allow.cartesian = TRUE)[, list(CATEGORY, CHID, ascadj)]
      }
      if (length(unique(calcats$CHOICE)) > length(unique(calcats$CHID))) {
        #the calibratin data need to be aggregated to the sim choices
        caldat <- merge(cal[CATEGORY %in% cats], calcats, "CHOICE")
        caldat <-
          caldat[, list(TARGET = sum(TARGET)), by = list(CATEGORY, CHID)]
    ...
    utils <-
      lapply(cats, function(y)
        sapply(1:alts, function(x)
          exp(pmin(
            rowSums(sweep(df[CATEGORY == y, mod[CHID == x &
                                                  CATEGORY == y, VAR], with = F], 2, mod[CHID == x &
                                                                                           CATEGORY == y, COEFF], "*")), 600
          ))))
    utils <-
      lapply(1:length(cats), function(x)
        if (is.null(dim(utils[[x]]))) {
          (utils[[x]] / sum(utils[[x]])) %*% ut
        } else {
          (utils[[x]] / rowSums(utils[[x]])) %*% ut
        })
    utils <- do.call("rbind", utils)
    temprand <- runif(max(df$Fin))
    simchoice <-
      unlist(lapply(1:nrow(df), function(x)
        1L + findInterval(temprand[df$Start[x]:df$Fin[x]], utils[x, ])))
  }
  return(simchoice)
}
...
create_pmg_inputs <- function(naics, g, sprod, recycle_check_file_path, log_file_path) {
  ...
  conscg <-
    consc[group == g, list(InputCommodity,
                           NAICS,
                           Zone,
                           Buyer.SCTG,
                           BuyerID,
  ...
  if (sprod == 1) {
    ...
    prodcg <-
      prodc[group == g, list(OutputCommodity,
                             NAICS,
                             Commodity_SCTG,
                             SellerID,
                             Size,
    ...
    consamount <-
      sum(conscg$PurchaseAmountTons) / sum(consc$PurchaseAmountTons)
    ...
    fwrite(prodc[, list(
      ...
      NAICS,
      Size,
      OutputCapacityTons = OutputCapacityTonsG,
      NonTransportUnitCost
    )],
    file = file.path(model$outputdir, paste0(naics, "_g", g, ".sell.csv")))
    prodcg <-
  ...
  if (nrow(pc[Commodity_SCTG %in% c(1:9)]) > 0) {
    ...
    pc_food <-
      pc[Commodity_SCTG %in% c(1:9), c("CATEGORY", unique(distchan_food$VAR[distchan_food$TYPE == "Variable"])), with =
           F]
    df <-
  ...
  if (nrow(pc[!Commodity_SCTG %in% c(1:9)]) > 0) {
    ...
    pc_mfg <-
      pc[!Commodity_SCTG %in% c(1:9), c("CATEGORY", unique(distchan_mfg$VAR[distchan_mfg$TYPE == "Variable"])), with =
           F]
    df <-
  ...
  pc[distchannel == 1 &
       Production_zone %in% c(179:180) &
       !Consumption_zone %in% c(179:180), distchannel := 3]				### -- Hawaii origin - non-Hawaii destination
  pc[distchannel == 1 &
       !Production_zone %in% c(179:180) &
       Consumption_zone %in% c(179:180), distchannel := 3]				### -- non-Hawaii origin - Hawaii destination
  pc[distchannel == 1 &
       Production_zone <= 273 &
       Consumption_zone > 273 &
       !Consumption_zone %in% c(310, 399), distchannel := 3]	### -- U.S. origin - foreign destination (except Canada/Mexico)
  pc[distchannel == 1 &
       Production_zone > 273 &
  ...
  if (nrow(pc[Commodity_SCTG %in% c(1:9)]) > 0) {
    ...
    pc_food <-
      pc[Commodity_SCTG %in% c(1:9), c("Commodity_SCTG", unique(ShipSize_food$VAR[ShipSize_food$TYPE == "Variable"])), with =
           F]
    df <-
      pc_food[, list(Start = min(.I), Fin = max(.I)), by = eval(c("Commodity_SCTG", unique(ShipSize_food$VAR[ShipSize_food$TYPE == "Variable"])))] #unique combinations of model coefficients
  ...
  if (nrow(pc[!Commodity_SCTG %in% c(1:9)]) > 0) {
    ...
    pc_mfg <-
      pc[!Commodity_SCTG %in% c(1:9), c("Commodity_SCTG", unique(ShipSize_mfg$VAR[ShipSize_mfg$TYPE == "Variable"])), with =
           F]
    df <-
      pc_mfg[, list(Start = min(.I), Fin = max(.I)), by = eval(c("Commodity_SCTG", unique(ShipSize_mfg$VAR[ShipSize_mfg$TYPE == "Variable"])))] #unique combinations of model coefficients
    setnames(df, "Commodity_SCTG", "CATEGORY")
    ####do this in the function (seems unecessary here)?
  ...
  for (i in unique(pc$Commodity_SCTG)) {
    pc[ship_size == 1 &
         Commodity_SCTG == i, weight := c(50L, 75L, 300L, 625L, 875L)[1 + findInterval(temprand, as.numeric(unlist(ship1[i])))]]
    pc[ship_size == 3 &
         Commodity_SCTG == i, weight := c(30000L, 75000L, 150000L)[1 + findInterval(temprand, as.numeric(unlist(ship3[i])))]]
  }
  ### --  for(i in unique(pc$Commodity_SCTG)){
