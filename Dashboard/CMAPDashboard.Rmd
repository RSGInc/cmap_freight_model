...
if (!require(RSGFAF)) {
  install.packages("../library/RSGFAF_4.3-1.zip", repos = NULL)
  library(RSGFAF)
} else {
  library(RSGFAF)
}
...
rsgcolordt <- data.table(red=c(246,0,99,186,117,255,82),
...
rsgcolordt[, hexValue := rgb(red,green,blue,maxColorValue=255)]

makeMoreColors <- colorRampPalette(rsgcolordt$hexValue, bias = 50) # Combines RSG colors to form more colors
...
modeCategories[, LinehaulMode := factor(LinehaulMode, levels = c("Truck", "Rail", "Water", "Air", "Multiple", "Pipeline", "Other", "None"), ordered = TRUE)]
...
buildDataTable <- function(data.formatted, container.sketch, type.currency = FALSE) {
  ...
  if (params$include.reference) {  dt <- datatable(data.formatted,
  ...
  dt
    ...
    cat("In order to see this table, please set include.reference = TRUE when you call rmarkdown::render.")
  }
}
...
generateDataColors <- function(table.nms, ref.table) {
  ...
  color.table.model <- data.table(Table = table.nms[Table != FAF.data.name, Table],
                                  Color = data.color.vec[1:table.nms[Table != FAF.data.name, .N]])
  if (params$include.reference) {
    color.table.FAF <- data.table(Table = FAF.data.name,
                                  Color = data.color.vec[4])
    color.table.ref <- data.table(Table = ref.table[!duplicated(Table) & 
                                                      !(Table %in% color.table.FAF[, Table]) &
                                                      !(Table %in% color.table.model[, Table]), Table],
                                  Color = data.color.vec[5])
    
    color.table.all <- rbind(color.table.FAF, color.table.model, color.table.ref)
    rm(color.table.FAF, color.table.model, color.table.ref)
  } else {
    color.table.all <- color.table.model
  }

  
  data.colors <- color.table.all[, Color]
  names(data.colors) = color.table.all[, Table]
...
prepDashModelData <- function(model.dt, skim.loc) {
  # Builds a ggplot2 graph comparing RSG model, FAF, and some other data source on a national or regional level
  #
  # Args:
  ...
  model.dt[, Move.Type := factor(Move.Type)]
...
prepDashFAFData <- function(year.selected) {
  ...
  faf.dt[, Move.Type := factor(Move.Type)]
...
if (length(params$model.run.names) < 1) {
  stop("Not enough model datasets given")
}
...
for (i in 1:length(params$model.run.names)) {
  ...
  dt <- prepDashModelData(get(load(data.loc)), skim.loc = skim.loc)
  dt[, Table := run.name]
...
##         data to also compare to. This same pattern is followed throughout the Modes
...
```{r build stacked tables for reference data}
...
### Ton-miles
```{r, echo=FALSE}
buildDataTable(data.formatted = DomesticCMAPCommodityFlows.TM, container.sketch = sketchDomesticCMAPCommodity, type.currency = FALSE)
```

### Tons
```{r,echo=FALSE}
buildDataTable(data.formatted = DomesticCMAPCommodityFlows.T, container.sketch = sketchDomesticCMAPCommodity, type.currency = FALSE)
```

### Values
```{r,echo=FALSE}
buildDataTable(data.formatted = DomesticCMAPCommodityFlows.V, container.sketch = sketchDomesticCMAPCommodity, type.currency = TRUE)
```
...
DomesticCommodityFlows <- table.stack[Trade.Type=="Domestic" & Table %in% c(primary.model.name, FAF.data.name),
                                      .(Tons=sum(Tons),Tonmiles=sum(Tonmiles),Values=sum(Values)),
                                      by=.(Move.Type,Mode.Domestic,SCTG,Table)][order(Move.Type,-Tons)]
# To reorder dcast output
DomesticCommodityFlows <- DomesticCommodityFlows[,Move.Type:=relevel(factor(Move.Type),ref="Within Zones")]
